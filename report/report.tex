...
ThreadScope spark information is shown in Figure \ref{fig:combinegrouped-sparks}. The sparks are also very small, most of them non-existent in the spark size output.
...
Figure \ref{fig:lines-fillorder-zoom} shows the same ThreadScope output, zoomed in to show some detail. The utilisation is very irregular with each spike representing a \texttt{DrawOrder} for one line.
...
Figure \ref{fig:lines-fillorder-spark} shows the spark output for the same run. We note the number of converted sparks corresponds to the number of lines in the program, indicating that they were all served by sparks.
...
The results from the previous section show that the current, non-parallel version performs better than any of our parallelisation attempts.\\

A frequent problem seemed to be that a large amount of sparks were garbage-collected or fizzled -- there simply was not enough work to pay off the debts of parallel overhead. Aside from inefficiency caused by flaws in our implementations, it seems that the granularity level of the functions inside the rasteriser is too small to be worth it. \\

Garbage collection was also part of the overhead, but a big part of that is likely due to strict evaluation. Rasterific is implemented using a high degree of laziness, allowing for a more consistent performance and memory usage throughout the lifetime of a program. With our parallelisation attempts, the forced evaluation of values quickly floods the heap, prompting more frequent cleanup. This also calls into question our decision to mainly use \texttt{Control.Monad.Par}, and not e.g. \texttt{Control.Parallel}.\\

We have not discussed the validity of our benchmarks. We do not currently know what the common use case for Rasterific is -- how many \texttt{DrawOrders} will there be, and how many \texttt{Primitives} - and by extension, \texttt{EdgeSamples} - will they each contain? This means something for where parallelisation will be of most benefit. Tracing the numbers in the benchmarks, we found the numbers shown in Figure \ref{fig:bench-numbers}.
...
Naturally, the most abundant elements are found at the lowest level of the rasteriser, but the computation needed for them is also comparatively much cheaper than at the higher levels. At the lowest levels, a better parallelisation approach would have been to chunk the work such that each spark would not finish instantly. Alternatively, to only evaluate the code in parallel if there was enough work to do. Given the unpredictable usage of Rasterific, we would probably benefit from the flexibility in dynamically choosing the best strategy.\\

Finally, the structure of Rasterific limits the amount of parallelism that can be achieved in any one level. For example, since the rasteriser is called once per high-level shape, we will only be able to gain performance as long as the cost of one shape is high, cf. Amdahl's law. As we saw in Section \ref{sec:fillorder}, parallelising at a higher granularity yields more apparent results. The scope for parallelism in Rasterific is by and large tree-shaped, and to gain the full benefits of parallelisation, we would need to flatten this tree.
